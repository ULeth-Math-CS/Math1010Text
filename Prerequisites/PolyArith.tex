...
\opengraphsfile{PolyArith}

\setcounter{footnote}{0}

\label{PolyArith}

In this section, we review the arithmetic of \textbf{polynomials}. What precisely is a polynomial?

\medskip

\colorbox{ResultColor}{\bbm

\begin{defn}  A \textbf{polynomial}\index{polynomial} is a sum of terms, each of which is either a real number, or a product of a real number and one or more variables raised to positive integer powers.
\end{defn}
...
Some examples of polynomials are $x^2 + x\sqrt{3} + 4$, $27x^2y + \frac{7x}{2}$ and $6$. Things like $3\sqrt{x}$, $4x - \frac{2}{x+1}$ and $13x^{2/3}y^{2}$ are \textbf{not} polynomials. (Do you see why not?) 

\medskip

\begin{rem}
In general, a polynomial can involve several variables. For example, $x^2y+3xyz-5yz^3$ is a polynomial in three variables, $x$, $y$, and $z$. However, in this text we will deal almost exclusively with polynomials of a single variable (usually denoted by $x$). A general polynomial in one variable is of the form
\[
a_nx^n+a_{n-1}x^{n-1}+\cdots+ a_2x^2+a_1x+a_0.
\]
(The subscript notation $a_0, a_1, a_2$, etc. for the coefficients is simply a bookkeeping device to make it easy for us to refer to the coefficients of each power of $x$.)
\end{rem}

\medskip

 Below we review some of the terminology associated with polynomials.
...
\item  \textbf{Coefficient:}   In the non-constant terms, the real number factor in the expression is called the \textbf{coefficient}\index{polynomial ! coefficient} of the term. 

\item  \textbf{Degree:}  The \textbf{degree}\index{polynomial ! degree} of a non-constant term is the sum of the exponents on the variables in the term; non-zero constant terms are defined to have degree $0$. The degree of a polynomial is the highest degree of the nonzero terms.\footnote{In the case of a single-variable polynomial, the degree is the highest power of $x$ with a non-zero coefficient.}

\item  \textbf{Like Terms:} Terms in a polynomial are called \textbf{like}\index{term ! like} terms if they have the same variables each with the same corresponding exponents.\footnote{For single-variable polynomials, like terms have the same power of $x$ in them.}
...
\begin{thm}\label{generaldistprop} \textbf{Generalized Distributive Property:}  To multiply a sum of $n$ terms by a sum of $m$ terms, multiply each of the $n$ terms of the first sum by each of the $m$ terms in the second sum and add the resulting $n \cdot m$ terms together. 
...
That is, we multiply the first term in the first sum by each of the terms in the second sum, and then the second term in the first sum, and so on.
...
The problem with multiplying a rule like ``F.O.I.L.'' is that it doesn't tell you what to do in any of the other cases covered by the generalized distributive property. For example, F.O.I.L. says nothing about the product $(a+b)(x+y+z)$, while Theorem \ref{generaldistprop} gives us
\[
...
\item  We begin `distributing the negative' as indicated on page \pageref{propertiesofnegatives} in Section \ref{RealNumberArithmetic}, then we rearrange and combine like term:\[\begin{array}{rclr}
...
\item Since exponents do \textbf{not} distribute across powers,\footnote{See the remarks following the Properties of Exponents on \pageref{propertiesofintegerexponents}.}  $\left(4w - \frac{1}{2} \right)^2 \neq (4w)^2 - \left(\frac{1}{2}\right)^2$.  (We know you knew that.)  Instead, we proceed as follows:\[ \begin{array}{rclr}
...
We now turn our attention to polynomial long division.  Dividing two polynomials follows the same algorithm, in principle, as dividing two natural numbers so we review\footnote{With apologies to those of you for whom this is not, in fact, review...} that process first.  Suppose we wished to divide $2585$ by $79$.  The standard division tableau is given below. 
...
\begin{thm} \label{polydiv0} \textbf{Polynomial Division:}  Let $d$ and $p$ be nonzero polynomials where the degree of $p$ is greater than or equal to the degree of $d$.  There exist two unique polynomials, $q$ and $r$, such that $p = d \cdot q + r,\,$ where either $r = 0$ or the degree of $r$ is strictly less than the degree of $d$.
\end{thm}
\ebm}
...
\item $(4y^4+3y^2+1) \div (2y^2-y+1)$\label{polydivexlast}
